...
library(patchwork) # for placing plots next to eachotherot
...
```{r}
ggplot(iris, aes(Petal.Width, Petal.Length, colour = Species)) + geom_point()

ggplot(iris, aes(Petal.Width, fill = Species)) +geom_histogram()
...
my_iris_plot <- ggplot(iris, aes(Petal.Width, Petal.Length, colour = Species)) + geom_point() 
...
__6. labs__ modifies the titles and names of the axis, legend, title etc.

```{r}

ggplot(iris, aes(Petal.Width, Petal.Length)) + geom_point()  + 
  labs(title = "Iris Petal Length to Petal Width", x = "Petal Width (cm)", y = "Petal Length (cm)") +
  mytheme3 +
